...
w0 = 3.*Gaussian(x, .2, .05);
w0 = normalize(w0);
w1 = 2.*Gaussian(x, .3, .05);
w1 = normalize(w1);
w2 = 5.*Gaussian(x, .5, .05);
w2 = normalize(w2);
w3 = 2.*Gaussian(x, .7, .05);
w3 = normalize(w3);
w4 = 2.*Gaussian(x, .8, .05);
w4 = normalize(w4);

od0 = tensorprod(w0,w2,2);
od0 = tensorprod(od0,w2,3);
od0 = tensorprod(od0,w4,4);

od1 = tensorprod(w2,w2,2);
od1 = tensorprod(od1,w3,3);
od1 = tensorprod(od1,w4,4);

od2 = tensorprod(w0,w3,2);
od2 = tensorprod(od2,w3,3);
od2 = tensorprod(od2,w4,4);

od3 = tensorprod(w4,w4,2);
od3 = tensorprod(od3,w2,3);
od3 = tensorprod(od3,w2,4);
